...
import customerModel from '../models/database/customers.model'
...
const CustomerService: ICustomerService = {
  ...
  async getData(req: Request) {
    ...
    const _data = await customerModel.find({ $and: conditions }).skip(skip).limit(limit).lean().exec()
    const count = await customerModel.countDocuments({ $and: conditions })
  ...
  async getDataById(req: Request) {
    try {
      ...
      const data = await customerModel.findById(dataId)
      return data
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async updateDataById(req: Request) {
    try {
      ...
      const data = req.body
      const updatedData = await customerModel.findByIdAndUpdate(dataId, data, { new: true })
      return updatedData
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async deleteDataById(req: Request) {
    try {
      const dataId = req.body.id ? req.body.id : req.params.id
      await customerModel.findByIdAndDelete(dataId)
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
    }
