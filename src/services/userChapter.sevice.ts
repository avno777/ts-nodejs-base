...
import userChapterModel from '../models/database/userChapter.model'
...
const CustomerService: IInventoryService = {
  async createData(data: any) {
    try {
      const createdData = await userChapterModel.create(data)
      return createdData
    } catch (error) {
  ...
  async getData(req: Request) {
    ...
    const _data = await userChapterModel.find({ $and: conditions }).skip(skip).limit(limit).lean().exec()
    const count = await userChapterModel.countDocuments({ $and: conditions })
  ...
  async getDataById(req: Request) {
    try {
      ...
      const data = await userChapterModel.findById(dataId)
      return data
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async updateDataById(req: Request) {
    try {
      ...
      const data = req.body
      const updatedData = await userChapterModel.findByIdAndUpdate(dataId, data, { new: true })
      return updatedData
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
  ...
  async deleteDataById(req: Request) {
    try {
      const dataId = req.body.id ? req.body.id : req.params.id
      await userChapterModel.findByIdAndDelete(dataId)
    } catch (error) {
      logger.error('Error creating data:', error) // Xử lý lỗi cụ thể
      throw error
    }
